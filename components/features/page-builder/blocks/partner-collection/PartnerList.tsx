...
/**
 * The "Parteneri" repeater for Partner Collection. Master-detail flow cloned
 * from `feature-cards/CardList`: a list of added partners, plus a separate
 * sub-form ("Partener nou" / edit) with its own draft buffer that only commits
 * on "Salvează partenerul". The card visual comes either from the preconfigured
 * icon palette or from an uploaded image — image handling matches
 * `people-grid/PersonList`.
 */
    ...
    return (
      <div className="rounded-xl border border-border p-4">
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor="pc-partener-nume" className={labelClass}>
          ...
          <div>
            <label htmlFor="pc-partener-subtitlu" className={labelClass}>
              Subtitlu
            </label>
            <input
              id="pc-partener-subtitlu"
              className={inputClass}
              value={draft.subtitlu}
              onChange={(e) => setField({ subtitlu: e.target.value })}
              placeholder="ex. Finanțator principal"
            />
          </div>
          ...
          {usesImage && draft.imagine ? (
            <div>
              <label htmlFor="pc-partener-alt" className={labelClass}>
              ...
              >
                ...
                >
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[#eff6ff] text-[#2563eb]">
                    {partner.sursaIcon === "imagine" && partner.imagine ? (
                      /* eslint-disable-next-line @next/next/no-img-element */
                      <img
